...
public abstract class RoomOverlay extends Overlay
{
    ...
    @Inject
    protected RoomOverlay(TombsOfAmascutConfig config) {
        this.config = config;
    ...
    public void renderTextLocation(Graphics2D graphics, String txtString, Color fontColor, Point canvasPoint)
    {
        if (canvasPoint != null)
        {
            graphics.setFont(new Font(FontManager.getRunescapeSmallFont().toString(), Font.BOLD, 11));
    ...
    {
        renderPoly(graphics, color, polygon, 1);
    }
